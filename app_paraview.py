...
def display_vtk_generation():
    """VTK生成タブ（ParaView API統合版）"""
    st.header("📦 VTKファイル生成とParaView可視化")
    ...
    # タブ作成
    tab1, tab2, tab3 = st.tabs(["📊 VTK生成", "🎨 ParaView可視化", "🔬 高度な可視化"])
    ...
    with tab2:
        st.subheader("🎨 ParaView可視化")
        
        # ParaViewレンダラーの初期化確認
        st.info("ParaView APIを使用した高品質レンダリング")
        
        # レンダラー選択
        renderer_type = st.selectbox(
            "レンダラータイプ",
            ["Simple (matplotlib)", "ParaView API", "両方で比較"],
            index=0,
            help="ParaView APIは事前インストールが必要です"
        )
        
        if renderer_type in ["ParaView API", "両方で比較"]:
            # ParaViewパス設定
            paraview_path = st.text_input(
                "ParaView Pythonパス（オプション）",
                placeholder="/usr/local/lib/python3.*/dist-packages/paraview",
                help="自動検出に失敗する場合は手動で指定"
            )
            
            # ParaViewレンダラー初期化
            pv_renderer = ParaViewRenderer(paraview_path if paraview_path else None)
            
            if not pv_renderer.paraview_available:
                st.warning("""
                ⚠️ ParaView APIが利用できません。以下を確認してください：
                1. ParaViewがインストールされている
                2. `pip install paraview` または ParaViewのPythonパスが正しい
                3. WSL環境の場合、X11サーバーが必要な場合があります
                """)
        
        # VTKファイルの選択（生成済みのもの）
        if st.session_state.get('generated_vtk_files'):
            st.subheader("📊 VTKファイル可視化")
            
            # ファイル選択
            vtk_files = list(st.session_state.generated_vtk_files.keys())
            selected_vtk = st.selectbox(
                "可視化するVTKファイル",
                vtk_files,
                key="viz_vtk_select"
            )
            ...
            if selected_vtk:
                vtk_info = st.session_state.generated_vtk_files[selected_vtk]
                vtk_path = vtk_info['vtk']
                ...
                with col1:
                    colormap = st.selectbox(
                        "カラーマップ",
                        ["viridis", "plasma", "inferno", "magma", "Cool to Warm", "rainbow"],
                        index=0
                    )
                    
                    show_scalar_bar = st.checkbox("スカラーバー表示", value=True)
                
                with col2:
                    resolution_width = st.number_input("幅", value=800, min_value=400, max_value=2000)
                    resolution_height = st.number_input("高さ", value=600, min_value=300, max_value=1500)
                
                with col3:
                    background = st.color_picker("背景色", value="#FFFFFF")
                    # HEXからRGBに変換
                    bg_rgb = tuple(int(background[i:i+2], 16)/255 for i in (1, 3, 5))
                
                # レンダリング実行
                if st.button("🎨 レンダリング実行", key="render_button"):
                    
                    if renderer_type == "Simple (matplotlib)" or renderer_type == "両方で比較":
                        # Simpleレンダラー
                        st.write("**matplotlib レンダリング:**")
                        simple_renderer = VTKSimpleRenderer()
                        
                        if simple_renderer.parse_vtk_file(vtk_path):
                            fig = simple_renderer.render_to_figure(
                                title=f"{vtk_info['lmr_type']}側 - 削孔軌跡",
                                colormap=colormap,
                                show_colorbar=show_scalar_bar,
                                figsize=(resolution_width/100, resolution_height/100)
                            )
                            st.pyplot(fig)
                            
                            # データサマリー
                            summary = simple_renderer.get_data_summary()
                            with st.expander("📊 データ情報"):
                                st.write(f"- ポイント数: {summary['num_points']}")
                                st.write(f"- ライン数: {summary['num_lines']}")
                                if summary['scalars']:
                                    st.write(f"- スカラーデータ: {', '.join(summary['scalars'])}")
                    
                    if renderer_type in ["ParaView API", "両方で比較"] and pv_renderer.paraview_available:
                        st.write("**ParaView API レンダリング:**")
                        ...
                        # ParaViewでレンダリング
                        with st.spinner("ParaViewでレンダリング中..."):
                            success = pv_renderer.render_vtk_file(
                                vtk_path=vtk_path,
                                output_path=output_path,
                                resolution=(resolution_width, resolution_height),
                                background_color=bg_rgb,
                                colormap=colormap if colormap != "viridis" else "Cool to Warm",
                            ...
                            if success:
                                ...
                                st.image(img, caption=f"ParaView レンダリング - {selected_vtk}")
                                
                                # VTK情報表示
                                vtk_info_data = pv_renderer.get_vtk_info(vtk_path)
                                if vtk_info_data:
                                    with st.expander("📊 VTK詳細情報"):
                                        st.write(f"- 境界: {vtk_info_data['bounds']}")
                                        st.write(f"- ポイント数: {vtk_info_data['number_of_points']}")
                                        st.write(f"- セル数: {vtk_info_data['number_of_cells']}")
                                        if vtk_info_data['point_data']:
                                            st.write(f"- ポイントデータ: {', '.join(vtk_info_data['point_data'])}")
                                
                                # 一時ファイル削除
                                os.unlink(output_path)
                            else:
                                st.error("ParaViewレンダリングに失敗しました")
        else:
            st.info("VTKファイルを生成してください（「VTK生成」タブ）")
    ...
    with tab3:
        ...
        if st.session_state.get('generated_vtk_files'):
            ...
            if len(st.session_state.generated_vtk_files) > 1:
                animation_files = st.multiselect(
                    "アニメーションに含めるファイル",
                    list(st.session_state.generated_vtk_files.keys()),
                    default=list(st.session_state.generated_vtk_files.keys())
                )
                
                fps = st.slider("FPS", min_value=1, max_value=30, value=10)
                
                if st.button("🎬 アニメーション作成"):
                    if 'pv_renderer' in locals() and pv_renderer.paraview_available:
                        vtk_file_list = [
                            st.session_state.generated_vtk_files[f]['vtk'] 
                            for f in animation_files
                        ]
                        
                        with st.spinner("アニメーション作成中..."):
                            output_anim = "output/animation.avi"
                            success = pv_renderer.create_animation(
                                vtk_file_list,
                                output_anim,
                                fps=fps
                            )
                            ...
                            if success:
                                st.success("アニメーションを作成しました")
                                # ダウンロードボタン
                                with open(output_anim, 'rb') as f:
                                    st.download_button(
                                        "⬇️ アニメーションをダウンロード",
                                        data=f.read(),
                                        file_name="drilling_animation.avi",
                                        mime="video/x-msvideo"
                                    )
                            else:
                                st.error("アニメーション作成に失敗しました")
                    else:
                        st.warning("ParaView APIが必要です")
            else:
                st.info("アニメーション作成には複数のVTKファイルが必要です")
            ...
            filter_file = st.selectbox(
                "フィルタを適用するファイル",
                list(st.session_state.generated_vtk_files.keys()) if st.session_state.get('generated_vtk_files') else [],
                key="filter_file_select"
            )
            
            if filter_file:
                available_filters = ["smooth", "decimate", "contour", "clip", "slice"]
                ...
                if st.button("🔧 フィルタ適用"):
                    if 'pv_renderer' in locals() and pv_renderer.paraview_available:
                        input_vtk = st.session_state.generated_vtk_files[filter_file]['vtk']
                        output_vtk = f"output/filtered_{filter_file.replace('.csv', '')}.vtk"
                        
                        with st.spinner("フィルタ適用中..."):
                            success = pv_renderer.apply_filters(
                                input_vtk,
                                output_vtk,
                                selected_filters
                            )
                            ...
                            if success:
                                ...
                                # ダウンロードボタン
                                with open(output_vtk, 'rb') as f:
                                    st.download_button(
                                        f"⬇️ フィルタ適用済みVTK",
                                        data=f.read(),
                                        file_name=Path(output_vtk).name,
                                        mime="application/octet-stream"
                                    )
                            else:
                                st.error("フィルタ適用に失敗しました")
                    else:
                        st.warning("ParaView APIが必要です")
                ...
                for file_name, info in st.session_state.generated_vtk_files.items():
                    ...
                    if Path(vtk_path).exists():
                        ...
                        st.download_button(
                            ...
                            key=f"download_vtk_{file_name}_tab3"
                        )
                ...
                for file_name, info in st.session_state.generated_vtk_files.items():
                    ...
                    if Path(csv_path).exists():
                        ...
                        st.download_button(
                            ...
                            mime="text/csv",
                            key=f"download_csv_{file_name}_tab3"
                        )
